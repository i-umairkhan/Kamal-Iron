...
def SaveData():
    ...
    df2 = pd.DataFrame([[str(date_time),date_, name.get(),address.get(),
                        phone1.get(),phone2.get(),
                        customerAdvanceDetails,Advance.get()
                        ]])
...
def searchfun2():
    global customerAdvance , customerAdvanceDetails
    ...
    for i in range(len(df)):
        if(df.loc[i,"Name"]==name.get()):
            ...
            total1 = Label(text= customerAdvanceDetails,padx=10,pady=3,background="#C4C4C4",font="height=20")
            total1.place(x=593,y=176)
            global CustomerRemaning
            CustomerAddress = df.iloc[0,3]
            CustomerPhone1 = int(df.iloc[0,4])
            CustomerPhone2 = (df.iloc[0,5])
            ...
            Advance.delete(0,END)
            Advance.insert(0,(customerAdvance))
            details = Label(text= customerAdvanceDetails,padx=10,pady=3,background="#C4C4C4",font="height=20")
            details.place(x=593,y=176)
            address.delete(0,END)
            phone1.delete(0,END)
            phone2.delete(0,END)
